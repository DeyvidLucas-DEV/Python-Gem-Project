...
async def test_create_subgrupo(client: AsyncClient):
...
async def test_create_subgrupo_without_description(client: AsyncClient):
...
async def test_update_subgrupo(client: AsyncClient, subgrupo_fix: Subgrupo):
    ...
    response = await client.put(f"{API_PREFIX}/{subgrupo_fix.id}", json=data)
    assert response.status_code == status.HTTP_200_OK
...
async def test_update_subgrupo_partial(client: AsyncClient, subgrupo_fix: Subgrupo):
...
async def test_update_subgrupo_duplicate_name(
    client: AsyncClient, db: AsyncSession, subgrupo_fix: Subgrupo
):
    ...
    response = await client.put(f"{API_PREFIX}/{outro_subgrupo.id}", json=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "já existe" in response.json()["detail"].lower()
...
async def test_delete_subgrupo(
    ...
    # Deleta
    response_del = await client.delete(f"{API_PREFIX}/{subgrupo_fix.id}")
    assert response_del.status_code == status.HTTP_204_NO_CONTENT

    # Verifica se foi deletado
    response_get = await client.get(f"{API_PREFIX}/{subgrupo_fix.id}")
    assert response_get.status_code == status.HTTP_404_NOT_FOUND


async def test_add_membro_to_subgrupo(
    client: AsyncClient, subgrupo_fix: Subgrupo, membro_fix: Membro
):
...
async def test_add_membro_subgrupo_not_found(client: AsyncClient, membro_fix: Membro):
    """Testa POST /{id}/membros/{membro_id} (Subgrupo não encontrado)"""
    response = await client.post(f"{API_PREFIX}/9999/membros/{membro_fix.id}")
...
async def test_add_membro_not_found_to_subgrupo(
    ...
    response = await client.post(f"{API_PREFIX}/{subgrupo_fix.id}/membros/9999")
...
async def test_upload_icone_subgrupo(client: AsyncClient, subgrupo_fix: Subgrupo):
...
async def test_upload_icone_invalid_type(client: AsyncClient, subgrupo_fix: Subgrupo):
...
async def test_upload_background_subgrupo(client: AsyncClient, subgrupo_fix: Subgrupo):
